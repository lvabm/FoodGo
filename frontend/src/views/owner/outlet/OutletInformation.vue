<template>
  <div class="px-10 py-8">
    ...
    <div v-else>
      <div v-if="outlets.length === 0" class="p-6 bg-white dark:bg-surface-dark rounded-lg">
        <p class="text-subtext-light dark:text-subtext-dark mb-4">Bạn chưa có quán nào. Hãy tạo quán mới để bắt đầu quản lý.</p>
        <button @click="openCreateOutlet" class="px-4 py-2 bg-primary text-white rounded-lg">Tạo quán mới</button>
      </div>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-1 bg-white dark:bg-surface-dark p-4 rounded-lg">
          ...
          <select v-model="selectedOutletId" @change="onSelectOutlet" class="w-full mt-2 p-2 border rounded">
          ...
          <div class="mt-4">
            ...
            <div class="mt-2 text-sm text-subtext-light dark:text-subtext-dark">
              ...
              <p><strong>Doanh thu (tạm):</strong> {{ stats.revenueDisplay }}</p>
              <p><strong>Đánh giá:</strong> <span v-if="selectedOutlet">{{ selectedOutlet.averageRating ?? 'Chưa có' }}</span><span v-else>—</span></p>
              <p><strong>Lượt đánh giá:</strong> {{ selectedOutlet?.totalReviews ?? 0 }}</p>
            </div>

            <div class="mt-4 flex gap-2">
              <button @click="viewOnSite" class="px-3 py-2 border rounded">Xem trang</button>
              <button @click="goToMenu" class="px-3 py-2 border rounded">Thực đơn</button>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-3 bg-white dark:bg-surface-dark p-6 rounded-lg">
          <div class="flex justify-between items-start">
            <div>
              <h2 class="text-xl font-bold mb-1">{{ selectedOutlet?.name || 'Không có thông tin' }}</h2>
              <p class="text-sm text-subtext-light dark:text-subtext-dark">{{ selectedOutlet?.address }}</p>
            </div>

            <div class="flex gap-2">
              <button @click="openEdit" class="px-4 py-2 bg-primary text-white rounded-lg">Chỉnh sửa</button>
              <button @click="toggleActive" class="px-4 py-2 border rounded" :disabled="isSaving">
          ...
          <div class="mt-6">
            ...
            <p class="text-subtext-light dark:text-subtext-dark">{{ selectedOutlet?.description || 'Chưa có mô tả' }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showEdit" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-surface-dark p-6 rounded-lg w-full max-w-2xl">
        <h3 class="text-lg font-semibold mb-4">Chỉnh sửa thông tin quán</h3>
        <div class="grid grid-cols-1 gap-3">
          <input v-model="editForm.name" placeholder="Tên quán" class="p-2 border rounded" />
          <input v-model="editForm.address" placeholder="Địa chỉ" class="p-2 border rounded" />
          <div class="grid grid-cols-2 gap-2">
            <input v-model="editForm.phoneNumber" placeholder="SĐT" class="p-2 border rounded" />
            <input v-model="editForm.email" placeholder="Email" class="p-2 border rounded" />
          </div>
          <div class="grid grid-cols-2 gap-2">
            <input v-model="editForm.priceRange" placeholder="Price range" class="p-2 border rounded" />
            <input v-model.number="editForm.capacity" placeholder="Capacity" class="p-2 border rounded" />
          </div>
        </div>

        <div class="mt-4 flex justify-end gap-2">
          <button @click="saveEdit" class="px-4 py-2 bg-primary text-white rounded" :disabled="isSaving">Lưu</button>
          <button @click="closeEdit" class="px-4 py-2 border rounded" :disabled="isSaving">Hủy</button>
        </div>
      </div>
    </div>
  </div>
...
const loadStats = async () => {
  ...
  try {
    ...
    const params = {page: 0, size: 1, outletId: selectedOutlet.value.id, viewType: 'MANAGE_BOOKINGS'};
    const res = await bookingApi.getMyBookings(params);
    const total = res?.totalElements ?? res?.data?.totalElements ?? (Array.isArray(res?.data) ? res.data.length : 0);
...
const goToMenu = () => {
  router.push({path: '/owner/menu', query: {outletId: selectedOutlet.value?.id}});
};
