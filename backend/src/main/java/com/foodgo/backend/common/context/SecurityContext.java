...
public final class SecurityContext {
  ...
  // Lấy đối tượng UserAccount đầy đủ từ SecurityContext. Đây là hàm nền tảng, các hàm khác sẽ gọi
  // lại hàm này.
  public static UserAccount getCurrentUserAccount() {
    Authentication authentication = SecurityContextHolder.getContext().getAuthentication();

    // Kiểm tra Authentication và trạng thái
    ...
    if (principal instanceof UserAccount userAccount) {
      // Lưu ý: Các quan hệ Lazy như 'bookings' sẽ không được tải.
      return userAccount;
    }
  ...
  // 4. Hàm tiện ích: Lấy Email
  public static String getCurrentUserEmail() {
    return getCurrentUserAccount().getEmail();
  }

  // 5. Hàm tiện ích: Kiểm tra xem User có Role cụ thể không
  public static boolean hasRole(String roleName) {
    return getCurrentUserAccount().getAuthorities().stream()
        // ROLE_NAME phải khớp với cách bạn lưu trong DB (ví dụ: "ADMIN", "USER")
        .anyMatch(a -> a.getAuthority().equals(roleName));
  }

  // 6. Hàm tiện ích: Lấy đối tượng UserDetails (nếu bạn muốn tương thích với các hàm Security
  // chuẩn)
